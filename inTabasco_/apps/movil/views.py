...
def nuevo_socio_pk(request,pk):
    ...
    if request.POST:
        print formulario_espacio_socio.is_valid()
        #print formulario_espacio_socio
        if formulario_espacio_socio.is_valid():
            #Edita al usuario para pasarle nombre, apellidos y correo#
            #consulta_usuario = User.objects.all().get(pk = pk)
            persona = cat_persona.objects.get(pk = pk)
